"""Remove only DRC-identified dangling AMP_SW_NEG copper in the C8 trial."""
...
for iteration in range(30):
 ...
 for v in d['violations']:
  for item in v['items']:
   uid=item['uuid'];t=lookup[uid];assert t.GetNetname()=='AMP_SW_NEG'
...
(D/'reports/c8-spur-removal.json').write_text(json.dumps({'removed_uuids':removed,'drc_findings':len(d['violations']),'unconnected_items':len(d['unconnected_items'])},indent=2))
