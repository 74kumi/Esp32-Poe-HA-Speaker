"""Remove only DRC-identified dangling RX signal copper in the RX termination trial."""
from pathlib import Path
import pcbnew as p,json,subprocess
D=Path('KiCad-RevE');board=D/'ethernet-trial.kicad_pcb';report=D/'reports/ethernet-trial-drc.json'
...
for iteration in range(30):
 ...
 for v in d['violations']:
  for item in v['items']:
   uid=item['uuid'];t=lookup[uid];assert t.GetNetname() in ['ETH_RX_P','ETH_RX_N']
...
(D/'reports/rx-termination-spur-removal.json').write_text(json.dumps({'removed_uuids':removed,'drc_findings':len(d['violations']),'unconnected_items':len(d['unconnected_items'])},indent=2))
