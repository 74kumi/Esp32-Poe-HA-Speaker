"""Remove only DRC-identified dangling AMP_SW_NEG copper in the C13 trial."""
...
for iteration in range(30):
 ...
 for v in d['violations']:
  for item in v['items']:
   uid=item['uuid'];t=lookup[uid];assert t.GetNetname() in ['24V_AMP','GND']
...
(D/'reports/c13-spur-removal.json').write_text(json.dumps({'removed_uuids':removed,'drc_findings':len(d['violations']),'unconnected_items':len(d['unconnected_items'])},indent=2))
