...
D=Path('KiCad-RevD');path=D/'PoE-Speaker-RevD.kicad_pcb';shutil.copy2(path,D/'reports/before-efuse-widths.kicad_pcb')
b=p.LoadBoard(str(path));changed={}
for t in b.GetTracks():
 if not isinstance(t,p.PCB_VIA) and t.GetNetname() in ['24V_OR','24V_AMP'] and t.GetLength()>2000000 and t.GetWidth()<800000:
...
(D/'reports/efuse-widths.json').write_text(json.dumps({'widened_to_mm':.8,'accepted_segments':len(changed),'scope':'Longer shared power segments only. Pin escapes, vias, thermal/current capacity still unqualified.'},indent=2));print('Accepted',len(changed),'widened power segments; unconnected:',len(d['unconnected_items']))
